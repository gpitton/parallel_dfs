...
from utils import hashdict, process_magma_output, sets_to_magma
...
def dfs_worker(q, marked, lock):
    ...
    while True:
        try:
            ...
            # We need to use a lock to prevent other threads
            # from changing marked as we are reading it.
            with lock:
                mkstr = sets_to_magma(marked.copy())
            cmd = ["magma", "-b", "s0:=" + s0str, f"n:={n}",
                   "mk:=" + mkstr, "dfs.m"]
            ...
            newsets = seen - marked
            print(f"{len(newsets)} new subsets.")
            for k in newsets:
